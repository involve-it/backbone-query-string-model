...
- Also, I have coded QueryStringCidModel - when this model is used querystring parameter will get prefix (cid of the query string model), so that multiple models can use same querystring names. Can be useful when several instances of the same control (like chart or table) are on the same page. Originally was developed to work with Backgrid. I can share it by request, please let me know! :)
